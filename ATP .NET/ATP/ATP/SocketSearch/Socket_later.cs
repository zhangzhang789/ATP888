...
namespace SocketSearch
{
    ...
    class Socket_later
    {
        ...
        ATPCurvePackage atpCurvePackage;
        ZCPackage zcPackage;
        DMIPackage dmiPackage;
        DCPackage dcPackage;
        ...
        public void SocketStart()
        {
            ...
            ATPToATPCurveClient = new UdpClient(new IPEndPoint(ATPIP, ATPPort));
            ATPToATPCurveClient.Connect(Convert.ToString(ATPCurveIP), ATPCurvePort);
            ATPToDMIClient = new UdpClient(new IPEndPoint(ATPIP, ATPPort));
            ATPToDMIClient.Connect(Convert.ToString(DMIIP), DMIPort);
            ATPToZCClient = new UdpClient(new IPEndPoint(ATPIP, ATPPort));
            ATPToZCClient.Connect(Convert.ToString(ZCIP), ZCPort);
            ATPToDCClient = new UdpClient(new IPEndPoint(ATPIP, ATPPort));
            ATPToDCClient.Connect(Convert.ToString(DCIP), DCPort);
        ...
        public void StartReceData()
        {
            Task.Run(() =>
            {
                ...
                while (true)
                {
                    byte[] buf = ATPToDMIClient.Receive(ref remoteEp);
            ...
            Task.Run(() =>
            {
                ...
                while (true)
                {
                    byte[] buf = ATPToZCClient.Receive(ref remoteEp);
                    Receive_ZC_Data(buf);
                }
            });
            Task.Run(() =>
            {
                ...
                while (true)
                {
                    byte[] buf = ATPToDCClient.Receive(ref remoteEp);
                    if (buf[2] == 6)
                    {
                        Receive_DC_Data(buf);   
                    }
        ...
        public void GetATPIPAndPort()
        {
            ...
            {
                if (item.DeviceName == "ATP")
                {
                    ATPIP = item.IP;
                    ATPPort = item.Port;
                }
                else if (item.DeviceName == "ZC")
                {
                    ZCIP = item.IP;
                    ZCPort = item.Port;
                }
        ...
        public UInt32[] SectionOrSwitchIDOFF(string balise) //寻找偏移量和distance_1
        {
            ...
            CurbaliseID = (byte)trainMessage.BaliseToIteam(balise).device.Id;
