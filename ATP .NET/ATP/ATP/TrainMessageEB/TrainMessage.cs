...
namespace TrainMessageEB
{
    class TrainMessage
    {
        ...
        {
            List<TopolotyNode> nodes_list = BaliseToIteam(curBalise).Right;
            return nodes_list;
        }

        public List<TopolotyNode> LeftNextCurBaliseList(string curBalise) //由当前的应答器右寻得到一个列表。当前是直轨下一个只有一个元素，当前是道岔则有两个元素
        {
            List<TopolotyNode> nodes_list = BaliseToIteam(curBalise).Left; //left本省就是一个列表
            return nodes_list;
        }
        ...
        {
            int LogicCount = (BaliseToIteam(curBalise).device as Section).LogicCount; //当是2时不是站台，有4个应答器
        ...
        {
            bool isUp = (BaliseToIteam(curBalise).device as RailSwitch).IsUp;
            bool isLeft = (BaliseToIteam(curBalise).device as RailSwitch).IsLeft;
