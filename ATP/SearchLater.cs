    ...
    class SearchLater
    {
        ...
        private int[] GetMAAndObstacleDistance(string curBalise, string MAEndLink, int MAEndOff, string[] obstacleID, byte[] obstacleState, int obstacleNum, bool isLeftSearch) //寻找障碍物的长度，距离
        {
            ...
            if (isLeftSearch == false) //右寻
            {
                while (true)
                {
                    List<线路绘图工具.TopolotyNode> NextNodesList = trainMessage.RightNextCurBaliseList(NowSearchBalise);
            ...
            else   //左寻
            {
                while (true)
                {
                    List<线路绘图工具.TopolotyNode> NextNodesList = trainMessage.LeftNextCurBaliseList(NowSearchBalise);
        ...
        public void GetObstacleLengthDis(List<线路绘图工具.TopolotyNode> NextNodesList, byte[] obstacleState, int[] obstacle_distance, string[] obstacle_name) //while循环用于寻路
        {
        ...
        public int[] GetObstacleLength(string[] obstacle_name, int[] obstacle_distance, int obstacleNum,ref int[] result_obstacle_length, ref int[] result_obstacle_distance)
        {
            ...
            {
                string nextObstacle = (trainMessage.BaliseToIteam(obstacle_name[i]).NodeDevice as RailSwitch).SectionName; //障碍物肯定是道岔
